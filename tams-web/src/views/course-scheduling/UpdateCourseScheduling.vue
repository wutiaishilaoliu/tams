<template>
  <el-dialog title="修改" width="790px" :close-on-click-modal="false" :close-on-press-escape="false" v-model="dialogVisible" :before-close="handleClose">
...
const attendTimeSelectKey = ref(Date.now())
const submitBtnLoading = ref(false)

const pickerOptions = { start: '07:00', step: '00:' + CONSTS.COURSE_DURATION_STEP_MINUTE, end: '20:00' }
...
const init = () => {
  attendTimeSelectKey.value = Date.now()
  Promise.all([
    classroomStore.getClassroomRefList(),
    courseStore.getCourseRefList(),
    teacherStore.getTeacherRefList()
  ]).then((res: any) => {
    classroomData.value = res[0]
    courseData.value = res[1]
    teacherData.value = res[2]
    search()
  }).catch(() => {})
}
...
const search = () => {
  csStore.getCourseSchedulingById(props.id).then((res: any) => {
    if (res) {
      ...
      courseDuration.value = form.value.duration || CONSTS.COURSE_DURATION_STEP_MINUTE
      currentCourse.value = { id: res.courseId, name: res.courseName, duration: res.duration }
    }
  }).catch(() => {})
}
...
const submit = () => {
  formRef.value?.validate((valid) => {
    if (valid) {
      ...
      csStore.updateCourseSchedulingById(props.id, form.value).then(() => {
